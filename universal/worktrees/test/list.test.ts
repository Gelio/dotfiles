...
import { sandbox, runEngine, writeCfg, makeRepo } from './helpers.ts';
...
test('list shows worktrees for a port-less repo (no port registry)', (t) => {
  ...
  writeCfg(root, repo, `export default { symlinkTargets: [] };`);
...
test('list --all spans registered repos', (t) => {
  ...
  writeCfg(root, r1, CFG);
  writeCfg(root, r2, CFG);
...
test('list --all survives a registered repo with a missing config', (t) => {
  ...
  fs.appendFileSync(reg, `${broken}\tmissing\n`);
